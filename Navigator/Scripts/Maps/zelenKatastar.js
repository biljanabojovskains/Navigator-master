...
var layersToAdd = zeleniloLayers.split(',');
var layersOff = zeleniloInactive.split(',');
var layers = [];
for (var layer in layersToAdd) {
    switch (layersToAdd[layer]) {
        ...
        case 'layerUlici':
            ...
            break;
        case 'layerZelenKatastar':
            layers.push(layerZelenKatastar);
            break;
        case 'layerDrva':
            layers.push(layerDrva);
            break;
        case 'layerCvekje':
            layers.push(layerCvekje);
            break;
        case 'layerZeleniPovrsini':
            layers.push(layerZeleniPovrsini);
            break;
...
for (var layer in layersOff) {
    switch (layersOff[layer]) {
        ...
        case 'layerUlici':
            ...
            break;
        case 'layerZelenKatastar':
            layerZelenKatastar.setVisible(false);
            break;
        case 'layerDrva':
            layerDrva.setVisible(false);
            break;
        case 'layerCvekje':
            layerCvekje.setVisible(false);
            break;
        case 'layerZeleniPovrsini':
            layerZeleniPovrsini.setVisible(false);
            break;
...
function goToGeojsonExtent2(id,coor) {
    ...
    vectorSource.clear();
    var myFeature = (new ol.format.GeoJSON()).readFeature("{ \"type\": \"Feature\",\"geometry\":{\"type\":\"Point\",\"coordinates\":["+x+","+y+"]}}");
    vectorSource.addFeature(myFeature);
    map.getView().fit(vectorSource.getExtent(), map.getSize());
}

function goToGeojsonExtentPolygon(id) {
    var data = $('#dataSearch').data(id);
    vectorSource.clear();
...
//function goToGeojsonExtent2(id,coordinates) {
//    var data = $('#dataSearch').data(id);
//    alert(coordinates);
//    vectorSource.clear();
//    var myFeature = (new ol.format.GeoJSON()).readFeature("{ \"type\": \"Feature\",\"geometry\":" + coordinates + "}");
//    vectorSource.addFeature(myFeature);
//    map.getView().fit(vectorSource.getExtent(), map.getSize());
//}
...
function goToGeojsonExtentPoint(id,geometry) {
    alert("responsot e " + id + "   " + geometry);
    
    $('divPopup').modal('hide');
    vectorSource.clear();
    var myFeature = (new ol.format.GeoJSON()).readFeature(geometry);
    vectorSource.addFeature(myFeature);
    map.getView().fit(vectorSource.getExtent(), map.getSize());
}
